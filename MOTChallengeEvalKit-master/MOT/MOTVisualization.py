...
class MOTVisualizer(Visualizer):
	...
	def drawResults(self, im = None, t = 0):
		...
		for bb in thisF:
			...
			# ids=((self.resFile[bb,10])).astype(int)
			...
			# if ids == 0:
			# 	color = self.colors[int(3 % len(self.colors))]
			# 	color = tuple([int(c*255) for c in color])
			# else:
			# 	color = self.colors[int(11 % len(self.colors))]
			# 	color = tuple([int(c*255) for c in color])
...
if __name__ == "__main__":
	visualizer = MOTVisualizer(
		seqName = "0013-0812",
		FilePath ="res/SportsMOT/MOT17-01.txt",
		image_dir = "data/SportsMOT/test/MOT17-01/0013",
		mode = "gt",
		output_dir  = "vid")
